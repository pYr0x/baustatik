...
import { angleInArc, normalizeAngle, sweepAngle, type Transformable } from './types'
...
export const Arc: Transformable<Arc> & {
  ...
  midpoint: (arc) => {
    ...
    return Point.make(arc.center.x + arc.radius * Math.cos(mid), arc.center.y + arc.radius * Math.sin(mid))
  },

  startPoint: (arc) =>
    Point.make(arc.center.x + arc.radius * Math.cos(arc.startAngle), arc.center.y + arc.radius * Math.sin(arc.startAngle)),

  endPoint: (arc) =>
    Point.make(arc.center.x + arc.radius * Math.cos(arc.endAngle), arc.center.y + arc.radius * Math.sin(arc.endAngle)),
  ...
  offset: (arc, distance) => {
    ...
    if (newRadius <= 0) throw new InvalidArcError(`Offset ${distance} erzeugt Radius ${newRadius} <= 0`)
    return { ...arc, radius: newRadius }
  },

  toPolyline: (arc, options = { tolerance: 0.1 }) => {
    if ('segments' in options && options.segments <= 0) throw new InvalidArcError(`segments ${options.segments} <= 0`)
    ...
    const segments =
      'segments' in options
        ...
        : Math.max(
            2,
            Math.ceil(
              sweep / Math.acos(Math.max(-1, Math.min(1, 1 - options.tolerance / arc.radius)))
            )
          )
    ...
    for (let i = 0; i <= segments; i++) {
      ...
      points.push(Point.make(arc.center.x + arc.radius * Math.cos(angle), arc.center.y + arc.radius * Math.sin(angle)))
    }
    return { points }
  },
  ...
  intersectArcFull: (a, b) => {
    ...
    if (d > a.radius + b.radius || d < Math.abs(a.radius - b.radius) || d < 1e-10) return []

    const cosA = (a.radius ** 2 + d ** 2 - b.radius ** 2) / (2 * a.radius * d)
    const baseAngle = Math.atan2(b.center.y - a.center.y, b.center.x - a.center.x)
    const alpha = Math.acos(Math.max(-1, Math.min(1, cosA)))

    if (alpha < 1e-10) {
      return [Point.make(a.center.x + a.radius * Math.cos(baseAngle), a.center.y + a.radius * Math.sin(baseAngle))]
    }

    return [alpha, -alpha].map((da) =>
      Point.make(a.center.x + a.radius * Math.cos(baseAngle + da), a.center.y + a.radius * Math.sin(baseAngle + da))
    )
  },
  ...
  intersectArc: (a, b) => {
    ...
    return Arc.intersectArcFull(a, b).filter((p) => {
      ...
      return angleInArc(angleA, a.startAngle, sweepA) && angleInArc(angleB, b.startAngle, sweepB)
    })
  },

...
export const Vector = {
  ...
  fromPoints: (a: Point, b: Point): Vector => ({ dx: b.x - a.x, dy: b.y - a.y }),
  ...
  subtract: (a: Vector, b: Vector): Vector => ({ dx: a.dx - b.dx, dy: a.dy - b.dy }),
  scale: (v: Vector, factor: number): Vector => ({ dx: v.dx * factor, dy: v.dy * factor }),

...
export const Line: Transformable<Line> & {
  ...
  parallel: (line, distance) =>
    Line.translate(line, Vector.scale(Line.normalVector(line), distance)),
  split: (line, point) => [
    Line.make(line.p1, point),
    Line.make(point, line.p2),
  ],
  ...
  angle: (a, b) =>
    Math.acos(
      Math.max(
        -1,
        Math.min(1, Vector.dot(Line.direction(a), Line.direction(b))),
      ),
    ),
  translate: (line, v) => ({
    p1: Point.translate(line.p1, v),
    p2: Point.translate(line.p2, v),
  }),

...
export const Polyline: Transformable<Polyline> & {
  ...
  fromLines: (lines) => {
    ...
    for (let i = 0; i < lines.length; i++) {
      const line = atOrThrow(lines, i);
      if (
        i > 0 &&
        !Point.equals(atOrThrow(points, points.length - 1), line.p1)
      ) {
  ...
  length: (pl) => {
    ...
    for (let i = 1; i < pl.points.length; i++)
      total += Point.distance(
        atOrThrow(pl.points, i - 1),
        atOrThrow(pl.points, i),
      );
    return total;
  },
  ...
  isClosed: (pl, tolerance = 1e-10) =>
    pl.points.length >= 3 &&
    Point.equals(
      atOrThrow(pl.points, 0),
      atOrThrow(pl.points, pl.points.length - 1),
      tolerance,
    ),
  ...
  toPolygon: (pl) => {
    ...
    if (points.length < 3)
      throw new InvalidPolygonError(
        'weniger als 3 Punkte nach Entfernung des Schlusspunkts',
      );
    const unique = points.filter(
      (p, i) => points.findIndex((q) => Point.equals(p, q)) === i,
    );
    if (unique.length < 3)
      throw new InvalidPolygonError(
        'weniger als 3 eindeutige Punkte nach Entfernung des Schlusspunkts',
      );
  ...
  pointAt: (pl, t) => {
    if (pl.points.length === 0)
      throw new InvalidPolylineError('pointAt auf leerer Polyline');
    if (pl.points.length === 1) return atOrThrow(pl.points, 0);
    ...
    for (let i = 1; i < pl.points.length; i++) {
      ...
      if (accumulated + segLen >= target) {
        ...
        const v = Vector.fromPoints(
          atOrThrow(pl.points, i - 1),
          atOrThrow(pl.points, i),
        );
        return Point.translate(
          atOrThrow(pl.points, i - 1),
          Vector.scale(v, localT),
        );
      }
      accumulated += segLen;
    }

    return atOrThrow(pl.points, pl.points.length - 1);
  },

  closestPoint: (pl, p) => {
    if (pl.points.length === 0)
      throw new InvalidPolylineError('closestPoint auf leerer Polyline');
    if (pl.points.length === 1) return atOrThrow(pl.points, 0);

    let closest = atOrThrow(pl.points, 0);
  ...
  split: (pl, point) => {
    if (pl.points.length < 2)
      throw new InvalidPolylineError(
        'split auf Polyline mit weniger als 2 Punkten',
      );
    ...
    const before = Point.equals(
      atOrThrow(beforeBase, beforeBase.length - 1),
      bestProjected,
    )
      ? [...beforeBase]
      : [...beforeBase, bestProjected];

    const after = Point.equals(atOrThrow(afterBase, 0), bestProjected)
      ? [...afterBase]
  ...
  translate: (pl, v) => ({
    points: pl.points.map((p) => Point.translate(p, v)),
  }),
  rotate: (pl, angle, origin) => ({
    points: pl.points.map((p) => Point.rotate(p, angle, origin)),
  }),
  mirror: (pl, axisP1, axisP2) => ({
    points: pl.points.map((p) => Point.mirror(p, axisP1, axisP2)),
  }),
};

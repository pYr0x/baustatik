...
interface ViewerConfig {
   ...
   *
   * Der Vorgänger `getSegments()` gab `Segment[]` heraus, einen Typ, den in
   * `src/` nie jemand konstruiert hat: der Viewer war sein einziger Verbraucher
   * und bekam ihn von außen hereingereicht. Mit `SectionGeometry` liest er
   ...
   * welche Kante überhaupt als Bogen gezeichnet wird, und sie steht seit
   * `schemaVersion: 7` im SELBEN Satz wie der Umriss, den der Viewer daneben
   * zeichnet. Eine Konstante hier zöge die Toleranz aus einer anderen Quelle
   * als den Satz; ein OPTIONALER Pull ließe die stille Abweichung nur
   * unauffälliger bestehen.
   */
...
export function createCrossSectionViewer(config: ViewerConfig) {
   ...
   * andere Eingabe ist, weiß der Viewer und nicht der Aufrufer. Eine Farbe ist
   * außerdem noch nicht der Anlass für eine View-Policy — ein
   ...
   *
   *   Der UMRISS kommt fertig aus dem Satz. Er ist bereits diskretisiert, trägt
   *   die Rundungen und stimmt mit den Zahlen überein, aus denen `A`, `Iy` und
   *   `Iz` fallen — genau dafür reist er mit. Der Viewer rechnet ihn nicht
   *   nach; täte er es, gäbe es zwei Umrisse und einen Streit darüber,
   *   welcher gilt.
   *
   ...
   *
   * EINE BOGENWAND WIRD JETZT MITGEZEICHNET (P1). Bis P0 gab `wallSpec` für
   * sie `undefined` zurück, weil `bulge` in einen `Arc` umzurechnen nicht
   * zweimal geschrieben werden durfte; seit `Bulge` in
   * `@baustatik/section-geometry` steht, gibt es die Umrechnung an einer
   * Stelle. Ihre SEHNE zu zeichnen wäre weiterhin die schlechtere Antwort:
  ...
  function sectionSpecs(geometry: SectionGeometry): Spec[] {
    const specs: Spec[] = geometry.outline
      // Ein Polygon unter drei Punkten trägt keine Fläche, und `render-core`
      // weist es zu Recht zurück. Das Gate lässt es trotzdem durch: es
      // fehlt erst, wenn KEIN Polygon trägt — während der Eingabe ist ein
      // halb gezogener Ring der Normalfall. Dieselbe Haltung wie bei den
      // Wänden: wer ein unfertiges Modell zeichnet, soll den Rest davon sehen.
   ...
   *
   * `undefined` heißt „hier nicht": ein hängender Verweis ist ein Befund des
   ...
   * keine Füllung (`render-core/src/specs.ts` trennt ihn deshalb vom
   * Ringsegment), und das ist für eine Mittellinie genau richtig.
   *
   ...
   * Mapping dazwischen ist `worldPoint(y, z)`, also die Identität. Gepinnt in
  ...
  function wallSpec(
    ...
    // t ist PHYSIK (die Wandstärke), nicht die Strichbreite am Schirm —
    ...
    // Knotenverweise der Wand — die heißen `startNodeId`/`endNodeId`.
    return {
      kind: 'line',
      id: wall.id,
      layer: 'section',
   ...
   * soll man SEHEN. Ein Wurf hier löschte Grid, Umriss und jede andere Wand
   * gleich mit, und weil `draw()` auch aus `onViewIntent` läuft, bräche er
   * mitten im Pan ab.
   *
   * DREI FAELLE FALLEN DESHALB AUF DIE SEHNE ZURUECK:
   *
   *   `bulge === 0`           — die gerade Wand, der Regelfall.
   *   `bulge` nicht endlich   — `Bulge.toArc` würfe `InvalidArcError`.
   *                             `NaN` käme sogar durch beide Vorprüfungen:
   *                             `NaN !== 0` ist wahr und `NaN <= tolerance`
   *                             falsch, die Kante gälte also als Bogen.
   *   `|Δ| >= 2π`             — `ArcPathSpec` verlangt `|sweepAngle| < 2π`
   *                             (`render-core/src/specs.ts`), und ab
   *                             `|bulge| ~ 1.6e16` rundet `4·atan(bulge)` genau
   *                             auf `2π`. Der Adapter wiese die Spec zurück.
   *
   * DAS GATE PRUEFT `bulge` HEUTE NICHT — G1 bis G6 sehen Umriss, doppelte
   * Ids, hängende Verweise, `t > 0`, Nulllänge und Knick, aber nie die
   * Wölbung selbst; die Knickwarnung rechnet bei `NaN` still `notch = NaN` und
   * schweigt. Beides kann also aus einem Store kommen, ohne dass irgendwer es
   * gemeldet hätte. Solange das so ist, fängt es der Zeichenweg ab.
   */
  function drawsAsArc(
    p1: PointType,
    p2: PointType,
    ...
    // der Viewer als Gerade. Ein eigenes Epsilon hier gäbe eine Kante, die
  ...
  function draw() {
    ...
    // folgt. Die z-Order GARANTIEREN aber die Bänder aus CROSS_SECTION_LAYERS,
    // die der Driver beim Aufbau bekommt — ohne sie landen beim Zoom-Out neu
    // gebaute Gridlinien über dem Querschnitt.
  ...
  // Kreis schließt sich intern: Intent -> neuer Viewport -> neu zeichnen.

 ...
 * hat.
 *
 * Sie bleibt, obwohl die duennbesetzte die Voreinstellung ist. Ein zweiter Weg
 * mit denselben Zahlen ist die einzige Probe, die ein Rechenkern gegen sich
 * selbst hat — und die Tests laufen ueber beide (ADR 0043).
 */
...
class DenseSystemMatrix implements SystemMatrix {
  ...
  async solve(
    ...
    for (let r = 0; r < size; r += 1) {
      const values = this.rows[free[r]];
      for (let c = 0; c < size; c += 1) {
        reduced[r * size + c] = values[free[c]];
      }
    }

/**
 * Die DÜNNBESETZTE Fassung: nur die Plätze, an denen etwas steht.
 *
 * WARUM SIE DIE VOREINSTELLUNG IST, und zwar aus Speicher- und nicht aus
 * Geschwindigkeitsgründen: 2 000 Knoten sind 6 000 Freiheitsgrade und damit
 * `36e6` Zahlen — 288 MB allein für `K`. Ein Stabwerk hat je Zeile etwa zwölf
 * Einträge; alles andere ist eine sehr große, sehr genaue Null (ADR 0043).
 *
 * SIE HÄLT DIE VOLLE, SYMMETRISCHE MATRIX und summiert Duplikate schon beim
 * `add`. Zwei Gründe, und beide zählen:
 *
 *   - `diagonal` und `rowDot` beantworten beide Fassungen damit GLEICH. `rowDot`
 *     braucht die volle Zeile einschließlich der Spalten gesperrter
 *     Freiheitsgrade; nur das untere Dreieck zu halten hieße, sie beim Lesen
 *     wieder zu spiegeln — an einer Stelle, an der ein Vorzeichenfehler nur in
 *     den Auflagerkräften auffiele.
 *   - Wir hängen nicht an faers ungeschriebener Duplikat-Semantik. Die
 *     Invarianten von `@baustatik/sparse-solver-wasm` sagen dazu nichts; was
 *     nicht zugesichert ist, wird nicht vorausgesetzt.
 *
 * Gefiltert wird erst beim Übergeben an den Port: dort fällt das obere
...
class SparseSystemMatrix implements SystemMatrix {
  ...
  async solve(
    ...
    // `-1` heißt „gesperrt": diese Zeile und diese Spalte fallen weg. `free`
    // ist aufsteigend, die reduzierte Nummerierung erhält also die
    // Reihenfolge — und damit bleibt „unteres Dreieck" dieselbe Aussage wie
    // global.
    const reducedOf = new Int32Array(this.n).fill(-1);
    for (let i = 0; i < size; i += 1) {
      reducedOf[atOrThrow(free, i)] = i;
    }

    // Zählen, dann füllen: die typisierten Arrays gehen so ohne
    // Zwischenkopie an den Port.
    let count = 0;
    for (let i = 0; i < size; i += 1) {
      const columns = this.rows.get(atOrThrow(free, i));

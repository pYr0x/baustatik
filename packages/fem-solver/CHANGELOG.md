...
- d3080d0: Die `AnalysisPolicy` verliert ihre eigene Schema-Version.
...
- Updated dependencies [2108d8a]
  ...
  - @baustatik/fem-load-resolve@0.0.4
  - @baustatik/fem-loads@0.0.4

## 0.0.3

### Patch Changes

- 1fda9e0: Two solver paths for the frame, and the sparse one is the default
  ...
  `solveSparseSystem` supply the capability. Which path runs is a _setting_ and
  persists; whether it is available is a _port_. See
  ...
  **The measurement runs against the productive factorizations.**
  ...
  second pivot column would only compare two routes through the _assembly_, and
  ...
  **Note on ADR numbering.** The plan asked only to _report_ the duplicated ADR
...
- Updated dependencies [d9a742d]
  ...
  - @baustatik/fem-load-resolve@0.0.3

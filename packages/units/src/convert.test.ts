...
describe('error handling', () => {
  ...
  it('should throw when input value is not a finite number', () => {
    expect(() => convert('abc' as any).from('m').to('cm')).toThrow()
    expect(() => convert('10' as any).from('m').to('cm')).toThrow()
    expect(() => convert(NaN).from('m').to('cm')).toThrow()
    expect(() => convert(Infinity).from('m').to('cm')).toThrow()
    expect(() => convert(-Infinity).from('m').to('cm')).toThrow()
    expect(() => convert(undefined as any).from('m').to('cm')).toThrow()
    expect(() => convert(null as any).from('m').to('cm')).toThrow()
    expect(() => convert(true as any).from('m').to('cm')).toThrow()
    expect(() => convert({} as any).from('m').to('cm')).toThrow()
  })

  it('should throw when unit is not supported', () => {
    expect(() => convert(1).from('banana' as any).to('cm')).toThrow()
    expect(() => convert(1).from('m').to('banana' as any)).toThrow()
    expect(() => convert(1).from('kW' as any).to('W' as any)).toThrow()
    expect(() => convert(1).from('Pa' as any).to('kPa' as any)).toThrow()
    expect(() => convert(1).from('J' as any).to('kJ' as any)).toThrow()
    expect(() => convert(1).from('foo' as any).to('bar' as any)).toThrow()
  })

  it('should throw when unit string is malformed or unparseable', () => {
    expect(() => convert(1).from('' as any).to('cm')).toThrow()
    expect(() => convert(1).from('m').to('' as any)).toThrow()
    expect(() => convert(1).from('!!!' as any).to('cm')).toThrow()
    expect(() => convert(1).from('m').to('###' as any)).toThrow()
    expect(() => convert(1).from('N//mm^2' as any).to('N/mm^2')).toThrow()
    expect(() => convert(1).from('N/mm^2').to('N//mm^2' as any)).toThrow()
    expect(() => convert(1).from('kN/m^' as any).to('N/m')).toThrow()
    expect(() => convert(1).from('MN/m^2^3' as any).to('N/m^2')).toThrow()
    expect(() => convert(1).from(123 as any).to('cm')).toThrow()
    expect(() => convert(1).from('m').to(123 as any)).toThrow()
    expect(() => convert(1).from(null as any).to('cm')).toThrow()
    expect(() => convert(1).from('m').to(null as any)).toThrow()
    expect(() => convert(1).from(undefined as any).to('cm')).toThrow()
    expect(() => convert(1).from('m').to(undefined as any)).toThrow()
  })

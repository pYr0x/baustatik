...
| `@baustatik/cross-section` | Section-value engine: `A`, `Iy`, `Iz`, `Iyz`, `Iu`, `Iv`, `alpha`, `ys`, `zs`, `yM`, `zM`, κ and `It` from four parametric shapes, a catalogue profile, or the editor's `SectionGeometry` — the last via Green on the carried outline (`calculation/green.ts`) plus, for a `thin-walled` `midline` graph with at most one cell, the positioned wall path (`calculation/wall-path/`, ADR 0040/0041: `S` from the wall model, `I` for κ from the outline). **Both** outline branches are derivable: `deriveOutline` is the one door, `deriveOutlineFromRings` and `deriveOutlineFromWalls` (run decomposition `branches` + inflate) sit behind it, `createSectionGeometry` is the factory. Owns `stressPoints` — thin-walled shapes and the catalogue profile only; every **solid** figure, parametric or drawn, answers `undefined`, because `t` and `S` are the denominator of a cut model and a solid section has none (ADR 0057). Owns the model record `CrossSection`, the creation policy `SectionPolicy` (`discretisationTolerance`, `principalAxisTolerance`, `miterLimit`, `thickWallRatio`, `shearCentreTolerance`, `FEElements`), and the warning gate `validateSectionGeometry`/`validateSectionProperties` (both take the policy; the gate re-derives the outline and reports drift — against the carried outline *and* against the FE fingerprint — and names multi-cell, disconnected and thick-wall figures). Owns `shapeOutline` (`geometry/shape-outline.ts`), which writes each parametric shape out as a `Ring[]` — axis-parallel, no `bulge`, the box as **two** rings — so that `idealisation: 'solid'` runs through the same FE as the drawn figure and κ, `It` and `yM`/`zM` stop being Grashof and `undefined` (ADR 0062). The closed formula stays and becomes the **oracle** of that outline; without a resolved FE block a parametric solid section is **shear-rigid**. Since [ADR 0064](docs/adr/0064-the-reinforcement-lives-on-the-cross-section.md) it also owns the **reinforcement**: `ReinforcementLayer` / `ReinforcementElement` and the optional field `reinforcement` on the `shape` and `section-geometry` variants of `CrossSection` — a layer **is** the Bewehrungsrang, an element is a position in mm plus `As` and an optional `Asmax` in cm², and **no material, grade or strength reaches the record** (`grep -r 'fy\|fck' src` still finds nothing). `sectionProperties` does not read the field and the section values do not move: the given `As` is the **starting value of an iteration**, not the reinforcement that will be built. Legal only on a Vollquerschnitt, which cannot be a type — `idealisation` sits one level below `reinforcement` — so it is the third gate door, `validateReinforcement(cs, policy)`, with six named findings; `isSolid`/`isSolidGeometry` are the one home of that switch, folded together from three written-out copies. Also **owns the types** of the solid-section FE (`FESectionValues`, `FESectionState`, the field `feValues` — on both `SectionGeometry` variants and, since ADR 0062, on the `shape` variant of `CrossSection` — and `kappaFromCoefficients`) while `@baustatik/cross-section-fe` does the computing — the package stays WASM-free by construction (ADR 0045/0047/0062). | `core`, `errors`, `section-geometry`, `steel-profiles`, `units` |
| `@baustatik/cross-section-fe` | The 2D FE of the **drawn solid** cross-section: `It`, the Trefftz shear centre, and κ as the ν-free coefficient pair `1/κ = d0 + d2·m²`, `m = ν/(1+ν)` (ADR 0045) — and, from the same solved fields, σ, τ and σv. **Two doors.** The asynchronous one, `computeFESectionValues(geometry, policy)` — **one geometry in, one result out, no ID**; deduplication is the application's, and its guard is the `feValues` field in the record. It answers a `kind`-discriminated union (`'refused'` \| `'solved'`), and the solved arm carries the mesh, the solved `FEFields` and the diagnostics alongside the record part, all transient (ADR 0039). The pure, synchronous one, `recoverStresses(fields, forces, nu)`, turns those fields into `FEStressField`: `nodes` (area-averaged, the verification shape, carries the boundary) and `elements` (one centroid value, the raw picture) out of **one** pass, plus jump, boundary-traction and reentrant-corner diagnostics. It **does not borrow `StressAtPoint`** and takes no dependency on `cross-section-stress` — τ at a node is a *vector*, not a flow along a tangent, and σv is shared as a formula (ADR 0061). `Mt` is **answered** here as Saint-Venant torsion (thrown next door), with the equilibrium closed over the **Weber** moment of the unit field and not over the Trefftz shear centre — so `Mt = 0` is not a torsion-free case for an unsymmetric figure. `nu` is mandatory and guarded to `[0, 0,5)`; no `Material` enters. Units: the outline arrives in mm, everything is computed in SI, the record part stays SI, and **only the stress door converts** — kN/kNm in, MPa and mm out (ADR 0024/0061). The one place that imports **both** WASM artifacts, which is why it is a package and not a folder in `cross-section` (ADR 0047, superseding 0046). Tri6, three quadratures chosen by integrand degree plus two sampling rules that are not quadratures, computed in the principal axes. **Both** boundary-value problems — torsion and shear — are solved for a displacement with a Neumann boundary, so they share one stiffness matrix and one factorization (five right-hand sides), and a hole anywhere costs nothing: no side condition and no limit (ADR 0048, amending 0045 on that point). Its suite meshes and solves for real and needs the built `pkg/` artifacts. Out of scope: **writing** a parametric shape out as a polygon — that is `shapeOutline` next door, and this package was deliberately left untouched when the parametric solid section joined it (ADR 0062; both input routes now agree bit for bit, `docs/messungen/t-querschnitt-grashof-gegen-fe.md`). Also out of scope: the thin-walled path, stress points, warping torsion, a selectable beam axis, a viewer, and any convergence loop. The ellipse — the one closed-form oracle with `ω ≠ 0` — is a named open proof site in its `CONTEXT.md`. | `core`, `cross-section`, `errors`, `mesh-2d-wasm`, `section-forces`, `sparse-solver-wasm`, `units` |
...
| `@baustatik/cross-section-viewer` | Viewer-facing cross-section composition and visualization. Six bands, `CROSS_SECTION_LAYERS` = `['grid','thin-walls','outlines','rebar','fe','symbols']`. The pure door is `crossSectionSpecs` (`scene.ts`); `viewer.ts` only pulls, holds the viewport and drives. Draws the outline carried in `SectionGeometry` rather than deriving one — in orange, because it is *derived* while the black wall centre lines are the *input*; arc walls become `arcPath` specs via `Bulge`. Results arrive through three optional pulls (`getProperties`, `getStressPoints`, `getFEMesh`) and an omitted pull is the off state; the `rebar` band sits between `outlines` and `fe` and is fed by a **fourth** pull, `getReinforcement`, which is **not** a result pull — `undefined` there means "no reinforcement", the normal case of every steel and timber section, and the element draws as a screen-constant marker whose radius comes **not** from `As` (ADR 0064). `CrossSectionFEMesh` is only *structurally* compatible with `Mesh2DResult`, so no mesher dependency. Owns `CrossSectionStyle` (the `Px` suffix = screen-constant; `Wall.t` stays physics) and the one exact m→mm conversion. | `cross-section`, `errors`, `grid-2d`, `render-core`, `section-geometry`, `units`, `viewport-2d` |
...
| `@baustatik/script` | Public browser-scripting DSL that builds serializable `schemaVersion: 15` model snapshots through model-owned handles. The snapshot is the **one** versioned record unit: it carries both project policies as mandatory fields — `sectionPolicy` since v7 and `analysisPolicy` since v13 — and each is parsed by its owner (ADR 0049). v14 puts the optional `feValues` block on the `shape` variant of `CrossSection` too (ADR 0062): a v13 file is unchanged in shape, but the MEANING of an absent block changed from "Grashof answers" to "shear-rigid until a run", and that is the break. v15 adds the optional `reinforcement` array to the `shape` and `section-geometry` cross-section variants (ADR 0064) — **the odd break out**: a v14 file is unchanged in shape *and* in meaning, it simply has no reinforcement, and it is still rejected because `exactKeys` is a whitelist and the repo rejects rather than migrates (ADR 0027). The `fem-solver` dependency exists for the `feValues` field alone; nothing here calls the solver. | `cross-section`, `errors`, `fem`, `fem-loads`, `fem-solver`, `material`, `steel-profiles` |
| `@baustatik/fem-section-resolve` | `CrossSection` × `Material` → `SectionStiffness`; the only place in the repo where geometry is multiplied by material: the drawn solid section stores κ as a formula, and `sectionStiffness` evaluates it with the beam material's `nu` (ADR 0045). It is **no longer the only place ν appears** — the FE stress recovery takes ν as a bare number (ADR 0061). What survives, and is the claim ADR 0045 actually makes: **no ν stands in the section values.** No `nu`, no κ — shear-rigid, which is the honest answer for timber. `SectionModel` carries `sectionPolicy` as a mandatory field — since ADR 0040 the wall path reads `discretisationTolerance`, so the record's own policy has to travel with it. | `cross-section`, `fem`, `fem-element`, `material` |

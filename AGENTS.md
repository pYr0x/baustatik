...
| `@baustatik/render-core` | Rendering abstractions shared by visual adapters, including the spec vocabulary. `SegmentSetSpec` is the mesh-agnostic wireframe primitive: flat `ArrayLike<number>` point and index buffers, so a typed array passes through without a copy. | `core`, `errors`, `viewport-2d` |

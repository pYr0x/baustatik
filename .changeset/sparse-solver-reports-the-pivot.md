---
'@baustatik/sparse-solver-wasm': patch
---

The sparse solver reports `pivotRatio` and `singularIndex`
...
- **`pivotRatio`** — the smallest pivot of the *scaled* matrix, reported **also
...
`unfixed` keeps its name and its meaning — *the matrix is not positive
definite*, not *mechanism*. Translating that into the language of a given field
